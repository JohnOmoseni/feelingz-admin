...
const emptyListVariants = cva("", {
	variants: {
		variant: {
			default:
				"md:min-h-[50vh] max-w-xl mx-auto gap-4 grid place-items-center min-h-[220px] w-full p-3",
		},
	},
	defaultVariants: {
		variant: "default",
...
const iconVariants = cva("object-contain", {
	variants: {
		iconVariant: {
			default: "",
			"show-icon": "w-fit= h-[200px] lg:h-[220px]",
		},
	},
	defaultVariants: {
		iconVariant: "default",
			...
			<div className={cn(emptyListVariants({}), containerStyles)}>
				<div className="flex-column items-center">
					{iconVariant === "show-icon" && (
						<Icon className={cn(iconVariants({ iconVariant }), className)} />
					)}

					<div
						className={cn(
							"flex-column gap-1.5",
							// iconVariant === "show-icon" && "-mt-[2rem]",
							footerSection && "mb-6"
						)}
					>
						<h3 className={cn("text-center font-semibold", titleStyles)}>
							{title || "No content"}
						</h3>
