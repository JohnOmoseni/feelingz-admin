...
export function DropdownList({
  ...
  const [loadingStates, setLoadingStates] = useState<boolean[]>(list!?.map(() => false));
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent className={cn("", containerStyles)}>
        {list && list?.length > 0 ? (
          list?.map((item, idx) => (
            <React.Fragment key={idx}>
              {idx !== 0 && showSeparator && (
                <DropdownMenuSeparator key="separator" className="h-px border border-border-100" />
              )}
              <DropdownMenuGroup>
                <DropdownMenuItem key={idx}>
                  {renderItem ? (
                    ...
                    >
                      ...
                      {loadingStates[idx] && <BtnLoader isLoading={loadingStates[idx]} />}{" "}
                    </div>
                  )}
                </DropdownMenuItem>
              </DropdownMenuGroup>
